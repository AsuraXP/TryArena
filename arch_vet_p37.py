...
P37b (--arm CF85R): s222 CF85 fell to .938/.674 with query_key_fp 1.0 —
the gate fired on queried keys and the (1-kg) read coupling from P35d
then suppressed exactly those reads (a bootstrap trap: a spurious write
at a hit position is harmless, but the coupling makes it fatal). Fix:
decouple — read_on = hit (structural, verified tag match), the gate
only controls writes. Prediction: s222 recovers to >= .95/.80.
"""
...
class KRBHind(p9.VETDCC):
    def __init__(self, V, d, slots, k=8, K=8, seen=True, cf=True, hseed=85, decouple=False):
        super().__init__(V, d, k=k, K=K); self.S = slots; self.seen = seen; self.cf = cf; self.decouple = decouple
    ...
    def forward(self, x):
        ...
        for t in range(L):
            ...
            rg = torch.sigmoid(self.Wread(torch.cat([s, xt], -1))); read_on = hit if self.decouple else hit * (1 - kg_prev).unsqueeze(-1)  # P37b: decoupled read
...
def main():
    ...
    for rn in a.jobs.split(","):
        ...
        torch.manual_seed(a.seed); m = KRBHind(V, 24, R["slots"], seen=True, cf=a.arm.startswith("CF"), hseed=(7 if a.arm == "CF" else 85), decouple=a.arm.endswith("R")); print(f"[p37] {rn}:KRB-HIND params {p19.n_params(m)}", flush=True)
