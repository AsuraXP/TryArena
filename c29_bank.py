...
ck = torch.load("c26r3_searched.pt")
...
print(f"[c29] bank cloned from entry state h{s_star}: {entry_count}", flush=True)
    ...
    if sc >= best - 1e-9:
        if sc > best + 1e-9:
            if sc - best >= 0.005 or sc >= 0.98:
                print(f"[c29] edit {edits} (macros {macros}): {best:.4f} -> "
                      f"{sc:.4f}", flush=True)
            no_improve = 0
        best = max(best, sc)
        visits = v2
        ...
        if kind == "macro":
            ...
            Eh[DIGS, h] = eold
        no_improve += 1
print(f"[c29] done: edits={edits} accepted={accepted} macros={macros} "
      f"best={best:.4f}", flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c29_bank.pt")
...
out = dict(tag="ARC2-C29-P6-BANK", search=dict(edits=edits, accepted=accepted,
