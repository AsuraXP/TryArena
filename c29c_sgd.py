...
def probe(kmax, dmax, n=64):
    ...
    for _ in range(n):
        ...
        ok += all(row[nd + 1 + 2 * i] == BLK and
                  row[nd + 2 + 2 * i] == DIG0 + digs[i] for i in range(nd))
    return ok, n
...
for (kmax, dmax) in stages:
    ...
    for s in range(600 if SMOKE else 4000):
        ...
        if s % 200 == 0 or SMOKE and s % 100 == 0:
            ...
            print(f"[c29c] stage k<={kmax} s{s} lr={lr:.5f} CE {loss.item():.4f} "
            ...
            if ok >= 60:
                ...
                break
    if not stag:
        print(f"[c29c] stage k<={kmax} did not crystallize", flush=True)
if best_sd is not None:
    m.load_state_dict(best_sd)
    print(f"[c29c] restored best-probe checkpoint (probe {best_ok}/64)", flush=True)
torch.save(m.state_dict(), "c29c_sgd.pt")
...
print(f"[c29c] rows changed vs counter seed: {len(changed)}", flush=True)
counter_rows = [(a, h) for a in (MARK, BLK, SEP) for h in range(H)]
counter_changed = [c for c in changed if (c[0], c[1]) in counter_rows]
print(f"[c29c] counter-region rows changed: {len(counter_changed)} "
...
out = dict(tag="ARC2-C29C-P6-SGD", cert=res, verdict=v, steps=step,
