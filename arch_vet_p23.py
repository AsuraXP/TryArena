...
def train_tf(name, model, pool, steps, batch, lr, warmup):
    """C76 tuned-TF trainer: AdamW + linear warmup + cosine decay to 10%; otherwise identical to p19.train_arm."""
    torch.manual_seed(0); opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=0.01); model.train(); t0 = time.time(); hist = []; n_pool = len(pool)
    sched = lambda st: (st / max(1, warmup)) if st < warmup else 0.1 + 0.9 * 0.5 * (1 + math.cos(math.pi * (st - warmup) / max(1, steps - warmup)))
    for step in range(1, steps + 1):
        ...
        lg = model(x[:, :256]); loss = F.cross_entropy(lg.reshape(-1, V), y.reshape(-1))
        opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0); opt.step()
        if step % 500 == 0: hist.append((step, round(loss.item(), 4))); print(f"  [{name}] step {step}/{steps} loss {loss.item():.4f} lr {lr*sched(step):.2e} ({time.time()-t0:.0f}s)", flush=True)
    return hist
...
def main():
    ...
    ap.add_argument("--lr", type=float, default=3e-3, help="C76: lr sweep for the TF control (3e-3 = all VET arms; 1e-3 / 3e-4 = tuned)")
    ap.add_argument("--warmup", type=int, default=0, help="C76: linear warmup steps for the TF control")
    a = ap.parse_args()
    t0 = time.time(); pool = joint_pool(); print(f"[p23] pool {len(pool)}", flush=True)
    out = {"tag": "ARCH-VET-LM-P23", "protocol": __doc__.split("ARMS")[0][-600:], "lr": a.lr, "warmup": a.warmup,
           "arms": {}}
    for arm in a.arms.split(","):
        ...
        for s in map(int, a.seeds.split(",")):
            ...
            hist = train_tf(f"P23-{arm}-s{s}-lr{a.lr}", m, pool, a.steps, 8, a.lr, a.warmup) if (a.lr != 3e-3 or a.warmup) else p19.train_arm(f"P23-{arm}-s{s}", m, pool, a.steps, 8)
            m.eval()
            with torch.no_grad():
