## STATE AT CYCLE 74 (read first)
Expert K (learned-predicate KRB) now matches the hand-wired ceiling:
arch_vet_p37.py --arm CF85R, R2 .981 n4 / .85 n8 on 3/3 seeds (consumed-
first eviction + balanced universal hash + structural read). Unified
system still carries the P35 K (.92/.72) -> C75 swaps in P37 K and re-
certifies 3 then 10 seeds. Generator audit clean; ./reproduce.sh quick
re-scores everything. Push auth intermittent: commit every result.
