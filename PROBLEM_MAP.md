...
## ARCH-VET P37 (cycle 74) — learned-predicate gap CLOSED at R2
- Consumed-first eviction (1 used bit/slot) + vocab-balanced hash +
  structural read: R2 n4 .981 x3, n8 .851/.847/.851 (hand-wired .865,
  oracle .842). Zero grammar in expert K. L-CONSUMED-FIRST-EVICTION,
  L-READ-PATH-MUST-BE-STRUCTURAL.
- Generator audit CLEAN (audit_generators.py). reproduce.sh added.
- OPEN QUEUE (C75): 1. fold P37 K into unified (3 -> 10 seeds); 2. R1
  n8 .79 diagnosis; 3. TF third seed + lr sweep on legacy axes; 4.
  overflow spill; 5. in-range parity; 6. fluency; 7. theory.

## ARCH-VET P36 + P37 K, 10-seed (cycle 75) — learned-predicate unified row CERTIFIED
- 6/6 bars on 10/10 seeds; MQAR n4 .983 sd .003 / n8 .855 sd .008 (was
  .919/.723 with the hand-wired predicate); other bars unchanged; gate
  cost 0 (routed == K-alone). L-LEARNED-PREDICATE-UNIFIED-10-SEED.
- R1 n8 .79 = cuckoo load-1.0 ceiling (L-CUCKOO-LOAD-CEILING), banked.

## TF control fairness (cycle 76) — optimizer-invariant ceiling
- 12 TF runs (3 seeds x NAPE/ALiBi x Adam-3e-3 / AdamW-1e-3-warmup-cos):
  best 2/4, modk at chance 12/12. L-TF-CEILING-OPTIMIZER-INVARIANT.
  Condition 1 (fair control) strengthened; full HP grid = C76b if asked.
- Groundbreakingness conditions: fair control ✓(strengthened), learned
  predicates ✓ (unified, 10 seeds), one unified model ✓ (10 seeds),
  third-party reproducibility: reproduce.sh + generator audit ✓, external
  run ✗.
