...
class Table3(nn.Module):
    """24-param organ: exact K-stack features -> (top,empty) readout table."""
    def __init__(self):
        super().__init__()
        self.table = nn.Parameter(0.1 * torch.randn(8, VOCAB))
    ...
    def features(self, x):
        """Minimal Markov state: (top, empty, prev_token_was_close)."""
        B, L = x.shape
        K = gE["CFG"]["K"]
...
print("\n" + "=" * 72)
print("TABLE3 (minimal Markov state) @4096 (total dCE | echo-dCE | open-CE)")
print("=" * 72)
for k, v in RESULTS.items():
    print(f"{k:<24} params {v['params']:<6} total {v['total']:<9} echo {v['echo']:<8} open {v['open']}", flush=True)
print("=" * 72)
final = {"tag": "ARC2-C10-TABLE3", "runs": RESULTS,
