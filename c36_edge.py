...
ck = torch.load("c34_depth.pt")
Ph, Eh, p0h = ck["Ph"].clone(), ck["Eh"].clone(), int(ck["p0h"])

# ---------- boundary-token cell set (d0/d9 + bdig0/bdig9 rows, all states)
repair_cells = set()
for a in (3, 12, 14, 23):
    for h in range(24):
...
print(f"[c36] census: {fail_cases}/{census_cases} failing; per-digit fails "
...
print(f"[c36] seed fitness = {best:.4f} val = {best_val:.4f}", flush=True)
    ...
    if sc >= best - 1e-9:
        ...
        if sc > best + 1e-9:
            if sc - best >= 0.002 or sc >= 0.98:
                print(f"[c36] edit {edits}: {best:.4f} -> {sc:.4f} "
        ...
        if which == 0:
            ...
            Ph[a, h] = old
        no_improve += 1
print(f"[c36] done: edits={edits} accepted={accepted} best={best:.4f}", flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c36_edge.pt")
