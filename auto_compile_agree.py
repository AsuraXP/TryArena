...
VAL = [gen_agree(24, 64, torch.Generator().manual_seed(7600 + i)) for i in range(3)]
def score(tab, Ph):
    c = t = 0
    for x, y, _, _ in VAL:
...
import os
CEMP = int(os.environ.get("CEMP", "0"))
for o_push in wops:
    for o_pop in [o for o in range(8, 15)]:   # pure-route pop (g=0), 15=identity
        ...
        for i in CLOSES:
            tab["o"][i] = o_pop; tab["b"][i] = CEMP; tab["q"][i] = s_lane
        ...
        with torch.no_grad():
            oldS0 = L0.S0.clone()
            L0.S0.copy_(L0.vcode[CEMP].unsqueeze(0).repeat(K, 1))
        s = score(tab, Ph)
        # greedy q/b polish
...
for step in range(1, 1501):
    x, y, _, _ = gen_agree(32, 64, g, max_depth=DREQ)
...
for L in (256, 1024, 4096):
    res[str(L)] = acc(gen_agree, L, 4, 9500)
out = dict(tag="EXP069-AGREE-COMPILED", ckpt=CKPT, acc=res,
           certified=all(v == 1.0 for v in res.values()),
           wall_s=round(time.time() - t0, 1),
           peak_rss_mb=round(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/1024,1))
print("RESULT " + json.dumps(out), flush=True)
open("results.jsonl", "a").write(json.dumps(out) + "\n")
torch.save(dict(tab=tab, Ph=Ph, model=model.state_dict()), "oppram_agree_compiled.pt")
