...
def fitness(Ph, Eh, p0h, cases, seed, w_cons, w_slot, w_order=0.0, w_sep=0.0):
    # w_order: gradual-consumption shaping — consumed fraction after pass p
    # should track p/(nd+1). Kills chaotic mass-conversion attractors.
    ...
    N = len(cases)
    w_cons = w_cons - w_order - w_sep
    return (0.10 * f_halt / N + 0.15 * f_disc / N + w_cons * f_cons / N
            + w_order * f_order / N + w_sep * f_sep / N
...
RESUME = os.path.exists("c26r4_searched.pt") and not SMOKE
if RESUME:
    ck0 = torch.load("c26r4_searched.pt")
    Ph, Eh, p0h = ck0["Ph"].clone(), ck0["Eh"].clone(), int(ck0["p0h"])
    print("[c26r4] RESUME mode: skipping stages A/B", flush=True)
else:
    ck0 = torch.load("c26r3_searched.pt")
    Ph, Eh, p0h = ck0["Ph"].clone(), ck0["Eh"].clone(), int(ck0["p0h"])
cases = cases_draw()
best, visits = fitness(Ph, Eh, p0h, cases, 0, 0.75, 0.0, 0.30, 0.25)
print(f"[c26r4] seed fitness(stage A) = {best:.4f}", flush=True)

# ---------------- Stage A: consumption ----------------
budgetA = 400 if SMOKE else 20000
edits = accepted = 0
no_improve = 0
cons_gate = 0.0
while not RESUME and edits < budgetA and no_improve < (400 if SMOKE else 15000):
    mv = R.random()
    undo = None
    if mv < 0.40:
    ...
    if edits % 25 == 0:
        cases = cases_draw()
    sc, v2 = fitness(Ph, Eh, p0h, cases, R.randrange(100000), 0.75, 0.0, 0.30, 0.25)
    edits += 1
    if sc >= best - 1e-9:
        if sc > best + 1e-9:
            if sc - best >= 0.005:
                print(f"[c26r4] A edit {edits}: {best:.4f} -> {sc:.4f}", flush=True)
            no_improve = 0
        best = max(best, sc)
        visits = v2
        accepted += 1
        ...
        if kind == "cell":
            ...
            Eh[DIGS, h] = eold
        no_improve += 1
print(f"[c26r4] Stage A done: edits={edits} best={best:.4f}", flush=True)

# ---------------- Stage B: + slot credit ----------------
bestB, visitsB = fitness(Ph, Eh, p0h, cases, 0, 0.15, 0.60, 0.10, 0.10)
budgetB = 400 if SMOKE else 80000
editsB = acceptedB = 0
no_improve = 0
while not RESUME and editsB < budgetB and no_improve < (400 if SMOKE else 20000):
    mv = R.random()
    undo = None
    if mv < 0.45:
    ...
    if editsB % 25 == 0:
        cases = cases_draw()
    sc, v2 = fitness(Ph, Eh, p0h, cases, R.randrange(100000), 0.15, 0.60, 0.10, 0.10)
    editsB += 1
    if sc >= bestB - 1e-9:
        if sc > bestB + 1e-9:
            if scB_delta := sc - bestB >= 0.005:
                print(f"[c26r4] B edit {editsB}: {bestB:.4f} -> {sc:.4f}",
                      flush=True)
            no_improve = 0
        bestB = max(bestB, sc)
        visitsB = v2
        ...
        if kind == "cell":
            ...
            Eh[DIGS, h] = eold
        no_improve += 1
print(f"[c26r4] Stage B done: edits={editsB} best={bestB:.4f}", flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c26r4_searched.pt")
...
for ndmax in stages:
    ...
    for s in range(1000 if SMOKE else 4000):
        ...
        if s % 200 == 0:
            ok, n = probe(ndmax)
            print(f"[c26r4] C stage nd<={ndmax} s{s} lr={lr:.5f} "
            ...
            if ok >= 60:
                ...
                break
    if not stag:
        print(f"[c26r4] C stage nd<={ndmax} did not crystallize", flush=True)
if best_sd is not None:
    m.load_state_dict(best_sd)
    print(f"[c26r4] restored best-probe checkpoint ({best_ok}/64)", flush=True)
if best_ok < 60:
    print("[c26r4] stage C fallback: per-pass trajectory curriculum (SCAFFOLD)",
          flush=True)
    def traj_loss(x, nds):
        B, T = x.shape
        p_tape = F.one_hot(x, A).float()
        ...
        for b in range(B):
            nd = nds[b]
            row = x[b].tolist()
            seq = [torch.tensor(row)]
            for p in range(1, nd + 2):
                prev = seq[-1].tolist()
                nxt = list(prev)
                # scaffold target: one more digit consumed+bound per pass
                for i in range(min(p, nd)):
                    nxt[src_pos(nd, i)] = BLK
                    nxt[tgt_pos(nd, i)] = row[src_pos(nd, i)]
                if p <= nd:
                    nxt[p - 1] = SEP
                seq.append(torch.tensor(nxt))
            y = torch.stack(seq[1:], 0)                  # (nd+1, T)
            pt = F.one_hot(x[b:b + 1], A).float()
            for p in range(nd + 1):
                elo = m.one_pass_soft(pt, 1, T)
                sm = F.softmax(elo, -1)
                hard = F.one_hot(elo.argmax(-1), A).float()
                pt = hard + sm - sm.detach()
                msk = (y[p] != PAD)
                ce = F.cross_entropy(elo[0][msk], y[p][msk])
                tot = ce if tot is None else tot + ce
        return tot / B
    for s in range(3000):
        step += 1
        x, nds = gen_batch(16, 4)
        loss = traj_loss(x, nds)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(m.parameters(), 1.0)
        opt.step(); opt.zero_grad()
        if s % 200 == 0:
            ok, n = probe(4)
            print(f"[c26r4] scaffold s{s} CE {loss.item():.4f} crisp-probe "
                  f"{ok}/{n}", flush=True)
            if ok > best_ok:
                best_ok = ok
                best_sd = {k: v.clone() for k, v in m.state_dict().items()}
    ...
    if best_sd is not None:
        m.load_state_dict(best_sd)
        print(f"[c26r4] post-scaffold best ({best_ok}/64)", flush=True)
torch.save(m.state_dict(), "c26r4_rebal.pt")
...
out = dict(tag="ARC2-C26R4-P6-REBAL",
