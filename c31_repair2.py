...
print(f"[c31] census: {fail_cases}/{census_cases} failing; per-digit fails "
...
print(f"[c31] seed fitness = {best:.4f}", flush=True)
    ...
    if sc >= best - 1e-9:
        if sc > best + 1e-9:
            if sc - best >= 0.002 or sc >= 0.98:
                print(f"[c31] edit {edits}: {best:.4f} -> {sc:.4f}", flush=True)
            no_improve = 0
        best = max(best, sc)
        accepted += 1
    else:
        if which == 0:
            ...
            Ph[a, h] = old
        no_improve += 1
print(f"[c31] done: edits={edits} accepted={accepted} best={best:.4f}", flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c31_repair2.pt")
...
out = dict(tag="ARC2-C31-P6-REPAIR2",
