...
def oracle_dec_k(digs, k):
    got = list(map(int, str(digs_val(digs) + 2 * k)))[::-1]
    while len(got) < len(digs):
        got.append(0)
    assert len(got) == len(digs) or len(got) == len(digs) + 1
    if len(got) > len(digs):  # overflow: keep low nd digits (wrap, certified below)
        got = got[:len(digs)]
    return got
...
add = torch.load("c24k_crispfix.pt")          # learned +1 digit organ
with torch.no_grad():
    m.P.copy_(F.one_hot(ck["Ph"], H).float() * 8 - 1)
    m.E.copy_(F.one_hot(ck["Eh"], A).float() * 8 - 1)
    m.p0[ck["p0h"]] = 8.0
    # organ clone: digit-phase rows of the ADD organ -> h11, clone to h12
    for a in range(A):
        m.E[a, 11] = add["E"][a, 11].clone()
        m.P[a, 11] = add["P"][a, 11].clone()
        m.E[a, 12] = add["E"][a, 11].clone()
        m.P[a, 12] = add["P"][a, 11].clone()
...
def gen_batch(batch, kmax, dmax):
    ...
    for _ in range(batch):
        ...
        nd = g.randrange(6, dmax + 1)          # nd>=6: 2k<=8 never overflows
...
def terminal_loss(x, ks):
    ...
    for b in range(B):
        ...
        gd = oracle_dec_k(digs, ks[b])
        sep = row.index(SEP)
        y[b, sep + 1:sep + 1 + len(gd)] = torch.tensor(gd) + DIG0
        y[b, :sep] = SEP  # dissolution protocol, as discovered
    msk = (y != PAD)
...
for (kmax, dmax) in stages:
    ...
    for s in range(600 if SMOKE else 4000):
        ...
        if s % 200 == 0 or SMOKE and s % 100 == 0:
            ...
            print(f"[c25bR] stage k<={kmax} s{s} lr={lr:.5f} CE {loss.item():.4f} "
            ...
            if ok >= 60:
                ...
                break
    if not stag:
        print(f"[c25bR] stage k<={kmax} did not crystallize", flush=True)
if best_sd is not None:
    m.load_state_dict(best_sd)
    print(f"[c25bR] restored best-probe checkpoint (probe {best_ok}/64)", flush=True)
torch.save(m.state_dict(), "c25bR_clone.pt")
...
print(f"[c25bR] rows changed vs counter seed: {len(changed)}", flush=True)
counter_rows = [(a, h) for a in (MARK, BLK, SEP) for h in range(H)]
counter_changed = [c for c in changed if (c[0], c[1]) in counter_rows]
print(f"[c25bR] counter-region rows changed: {len(counter_changed)} "
...
res = {}
certs = dict(indist=(2, 10, 500), k16=(16, 40, 200), k64=(64, 40, 100),
             joint=(64, 120, 100))
if SMOKE:
    certs = dict(indist=(2, 10, 40))
for name, (k, nd, n) in certs.items():
    gp = random.Random(2500 + k)
...
if not SMOKE:
    v.update(S2=res["k16"]["exact"], S3=res["k64"]["exact"], S4=res["joint"]["exact"],
...
out = dict(tag="ARC2-C25BR-P3-CLONE", cert=res, verdict=v, steps=step,
