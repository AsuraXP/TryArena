...
for (kmax, dmax) in stages:
    ...
    for s in range(4000):
        ...
        if s % 200 == 0:
            ...
            print(f"[c24j] stage k<={kmax} s{s} lr={lr:.5f} CE {loss.item():.4f} "
            ...
            if ok >= 60:
                ...
                break
    if not stag:
        print(f"[c24j] stage k<={kmax} did not crystallize in 4000 steps", flush=True)
if best_sd is not None:
    m.load_state_dict(best_sd)
    print(f"[c24j] restored best-probe checkpoint (probe {best_ok}/64)", flush=True)
torch.save(m.state_dict(), "c24j_decay.pt")
...
out = dict(tag="ARC2-C24J-P4-DECAY", cert=res, verdict=v, steps=step,
