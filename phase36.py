...
FREEZE = True
STAGE = sys.argv[2] if len(sys.argv) > 2 else "A"      # A=plain, B=modal fine-tune
LOAD = sys.argv[3] if len(sys.argv) > 3 else None
...
class LMKR(nn.Module):
    def __init__(self, vin=6, vout=6, d_model=32, k=12, d_slot=16, M=4):
        ...
        mask = torch.zeros(16); mask[9:] = -1e9        # forbid route ops 9-15
        self.register_buffer("amask", mask)
    ...
    def forward(self, x):
        ...
        for t in range(L):
            ...
            a = F.softmax(self.alpha(hc) + self.amask, -1)
...
t0 = time.time()
NSTEP = 12000 if STAGE == "A" else 6000
PM = 0.0 if STAGE == "A" else 0.1
for step in range(1, NSTEP + 1):
    L = 32 if step < NSTEP // 2 else 64
    x, y, _ = gen_modal_lm(32, L, g, p_mode=PM)
    loss = F.cross_entropy(model(x).reshape(-1, 6), y.reshape(-1))
    loss.backward()
    torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
    opt.step(); opt.zero_grad()
    if step % (NSTEP // 2) == 0: print(f"[lmkr-s{SEED}] {step} CE {loss.item():.5f}", flush=True)
torch.save(model.state_dict(), f"lmkr_w_{STAGE}_s{SEED}.pt")
model.eval(); res = {}
with torch.no_grad():
        ...
        for L in (64, 1024, 4096):
            ...
            for i in range(3):
                x, y, o = gen_modal_lm(4 if L > 256 else 16, L,
                                       torch.Generator().manual_seed(9770 + L + i), p_mode=PM)
...
out = dict(tag=f"EXP110-LMKR-{STAGE}-S{SEED}", dCE=res, wall_s=round(time.time()-t0,1),
