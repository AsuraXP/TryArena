"""
ARC-2 CYCLE 21 / LM HOST — the chatbot axis: real-text language modeling
...
CFG = dict(steps=12000, batch=32, train_len=256, ckpts=[3000, 6000, 9000, 12000],
           d_model=32)
...
if os.environ.get("RESUME") == "1":
    existing = [c for c in CFG["ckpts"]
                if os.path.exists(os.path.join(HERE, f"lm_host_{c}.pt"))]
    if existing:
        last = max(existing)
        m.load_state_dict(torch.load(os.path.join(HERE, f"lm_host_{last}.pt")))
...
for step in range(START, CFG["steps"] + 1):
    ...
    if step in CFG["ckpts"] and step not in ckpts_done:
        torch.save(m.state_dict(), os.path.join(HERE, f"lm_host_{step}.pt"))
        ckpts_done.add(step)
...
torch.save(m.state_dict(), os.path.join(HERE, "lm_host_final.pt"))

print("[eval] val CE by length, per ckpt:", flush=True)
for c in CFG["ckpts"]:
    m.load_state_dict(torch.load(os.path.join(HERE, f"lm_host_{c}.pt")))
    eval_all(m, f"lm_{c // 1000}k", RESULTS)
m.load_state_dict(torch.load(os.path.join(HERE, "lm_host_final.pt")))
