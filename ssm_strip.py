...
def gen_dyck10(batch, length, rng, D=10):
    L2, L4 = math.log(2.0), math.log(4.0)
    xs, ys, os_ = [], [], []
    for _ in range(batch):
        x, nll, stack = [], [], []
...
V_MOD7 = 7
VOCAB = V_MARKOV + V_DYCK + V_MOD7

def gen_mod7(batch, length, rng):
    """Deterministic mod-7 walk: x_t = (x_{t-1} + r) mod 7, r in {1,2,3}.
    Next token fully determined by count state -> oracle CE = 0, dCE = CE.
    The documented transformer failure zone (Hahn 2020)."""
    xs, ys, os_ = [], [], []
    for _ in range(batch):
        x = [rng.randrange(V_MOD7) + V_MARKOV + V_DYCK]
        for _ in range(length):
            r = rng.randrange(1, 4)
            x.append((x[-1] + r) % V_MOD7 + V_MARKOV + V_DYCK)
        # r in {1,2,3} => 3 equiprobable successors => oracle CE = ln 3
        xs.append(x[:length]); ys.append(x[1:length + 1])
        os_.append([math.log(3.0)] * length)
    return torch.tensor(xs), torch.tensor(ys), torch.tensor(os_)

TASKS = {"dyck10": gen_dyck10, "mod7": gen_mod7}
...
def train(model, seed, tag):
    ...
    for step in range(1, CFG["steps"] + 1):
        xd, yd, _ = gen_dyck10(CFG["batch"] // 2, CFG["train_len"], rng)
        xm, ym, _ = gen_mod7(CFG["batch"] // 2, CFG["train_len"], rng)
...
def make_variants():
    ...
    class S32(SSMHost):
        def __init__(self):
            ...
            self.emb = nn.Embedding(VOCAB, d)
            nn.init.normal_(self.emb.weight, std=0.02)
            self.blocks = nn.ModuleList([SSMBlock(d) for _ in range(2)])
            self.norm = nn.LayerNorm(d)
            self.head = nn.Linear(d, VOCAB)
            self.head.weight = self.emb.weight
    class S16(SSMHost):
    ...
    class S16_1(SSMHost):
        def __init__(self):
            ...
            self.head = nn.Linear(d, VOCAB)
            self.head.weight = self.emb.weight
    out += [("ssm_d32_2", S32), ("ssm_d16_2", S16), ("ssm_d16_1", S16_1)]
    return out

for name, ctor in make_variants():
    torch.manual_seed(0)
...
# fresh same-budget RoPE attention control (new tasks => no old checkpoints apply)
...
train(tf, 0, "tf_rope-s0")
ALL["tf_rope"] = {f"{t}@{L}": round(eval_dce(tf, t, L, CFG["eval_reps"]), 4)
                 for t in TASKS for L in CFG["eval_lens"]}
...
print("=" * 100)
beats = {f"{t}@{L}": (ALL["ssm"][f"{t}@{L}"] < ALL["tf_rope"][f"{t}@{L}"])
         for t in TASKS for L in CFG["eval_lens"]}
verdict = (f"ssm beats tf_rope on {sum(beats.values())}/{len(beats)} cells; "
           f"mod7@4096: ssm {ALL['ssm']['mod7@4096']} vs tf {ALL['tf_rope']['mod7@4096']}; "
           f"dyck10@4096: ssm {ALL['ssm']['dyck10@4096']} vs tf {ALL['tf_rope']['dyck10@4096']}")
final = {"tag": "ARC2-C8-LINATT-HARD", "runs": ALL, "beats_tf": beats, "verdict": verdict,
