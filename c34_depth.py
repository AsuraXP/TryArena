...
for _ in range(census_cases):
    nd = gc.randrange(2, 5) if gc.random() < 0.5 else gc.randrange(6, 17)
...
print(f"[c34] census: {fail_cases}/{census_cases} failing; per-digit fails "
...
val_cases = [2, 3, 4, 4, 6, 8, 12, 16]
best = fitness(Ph, Eh, p0h, cases, 0)
best_val = fitness(Ph, Eh, p0h, val_cases, VAL_SEED)
print(f"[c34] seed fitness = {best:.4f} val = {best_val:.4f}", flush=True)

budget = 400 if SMOKE else 60000
...
while edits < budget and no_improve < no_improve_cap:
    if R.random() < 0.25:
        d = R.choice([0, 1, 4, 7, 9])
        a = DIG0 + d
        (which, a_, h) = R.choice([(c) for c in repair_cells if c[1] == a]
                                  or [(0, a, 11)])
        v = R.choice(BANK)
        undo_cells = [(0, a, h, int(Eh[a, h])), (1, a, h, int(Ph[a, h])),
                      (0, BLK, v, int(Eh[BLK, v])), (1, BLK, v, int(Ph[BLK, v]))]
        Eh[a, h] = BLK
        Ph[a, h] = v
        Eh[BLK, v] = BDIG0 + d
        Ph[BLK, v] = 13
        ...
        if sc >= best - 1e-9:
            ...
            if sv < best_val - 1e-9:
                ...
                no_improve += 1
                continue
            best = max(best, sc); best_val = max(best_val, sv); accepted += 1
            no_improve = 0
        else:
            for (w2, a2, h2, o2) in undo_cells:
                if w2 == 0:
    ...
    if sc >= best - 1e-9:
        ...
        if sc > best + 1e-9:
            if sc - best >= 0.002 or sc >= 0.98:
                print(f"[c34] edit {edits}: {best:.4f} -> {sc:.4f} "
        ...
        if which == 0:
            ...
            Ph[a, h] = old
        no_improve += 1
print(f"[c34] done: edits={edits} accepted={accepted} best={best:.4f}", flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c34_depth.pt")
...
out = dict(tag="ARC2-C34-P6-DEPTH",
