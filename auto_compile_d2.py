...
OPENS, CLOSES, PEEKS = [0, 2], [1, 3], [4, 5, 6]
torch.manual_seed(0)
...
VAL = [gen_dyck2p(24, 64, torch.Generator().manual_seed(7600 + i)) for i in range(3)]
def score(tab, Ph):
    c = t = 0
    for x, y, _, _ in VAL:
...
for step in range(1, 1501):
    x, y, _, _ = gen_dyck2p(32, 64, g, max_depth=DREQ)
...
for L in (256, 1024, 4096):
    res["peek" + str(L)] = acc(gen_dyck2p, L, 4, 9500)
    res["plain" + str(L)] = acc(gen_dyck2, L, 4, 9900)
out = dict(tag="EXP064-AUTOCOMPILE-V2-DYCK2P", ckpt=CKPT, acc=res,
           certified=all(v == 1.0 for v in res.values()),
           wall_s=round(time.time() - t0, 1),
           peak_rss_mb=round(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/1024,1))
print("RESULT " + json.dumps(out), flush=True)
open("results.jsonl", "a").write(json.dumps(out) + "\n")
torch.save(dict(tab=tab, Ph=Ph, model=model.state_dict()), "oppram_dyck2p_compiled.pt")
