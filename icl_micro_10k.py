...
VOCAB = NK + NV
LN16 = math.log(16.0)
CFG = dict(steps=10000, batch=32, train_len=64,
...
for name, mk in [("ssm_d16_1", lambda: SSMHost(16, 1)),
    ...
    torch.save(m.state_dict(), f"icl10k_{name}_s0.pt")
    ALL[name] = {f"L{L}": eval_dce(m, L, CFG["eval_reps"]) for L in CFG["eval_lens"]}
    del m

print("\n" + "=" * 84)
print("ICL-MICRO  dCE nats/token (0 = oracle) | target-dCE = CE on the FINAL value")
print("=" * 84)
...
print("=" * 84)
final = {"tag": "ARC2-C9-ICL-MICRO-10K", "runs": ALL,
