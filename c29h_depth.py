...
def fitness(Ph, Eh, p0h, cases, seed):
    ...
    for nd in cases:
        ...
        if cstates:
            f_sep += min(1.0, len(set().union(*cstates.values()))
                         / max(len(cstates), 1))
        if len(tr) > 1:
...
ck = torch.load("c29f_bdig.pt")  # resume best bdig tables
...
print(f"[c29h] bank cloned from entry state h{s_star}: {entry_count}", flush=True)
...
print(f"[c29h] seed fitness = {best:.4f}", flush=True)
    ...
    if sc >= best - 1e-9:
        if sc > best + 1e-9:
            if sc - best >= 0.005 or sc >= 0.98:
                print(f"[c29h] edit {edits} (macros {macros}): {best:.4f} -> "
                      f"{sc:.4f}", flush=True)
            no_improve = 0
        best = max(best, sc)
        visits = v2
        ...
        if kind == "macro":
            ...
            Eh[fam, h] = eold
        no_improve += 1
print(f"[c29h] done: edits={edits} accepted={accepted} macros={macros} "
      f"best={best:.4f}", flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c29h_depth.pt")
...
out = dict(tag="ARC2-C29H-P6-DEPTH", search=dict(edits=edits, accepted=accepted,
