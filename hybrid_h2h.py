"""
HYBRID HEAD-TO-HEAD: Transformer vs Register-Machine vs Hybrid (Transformer+Coprocessor)
=========================================================================================
...
ARCHS = {
    ...
    "machine":     lambda: MachineLM(CFG["d_model"], CFG["k"], CFG["d_slot"]),
    "hybrid":      lambda: HybridLM(CFG["d_model"], CFG["k"], CFG["d_slot"]),
}
ALL = {}
t_start = time.time()
for seed in CFG["seeds"]:
    for name, ctor in ARCHS.items():
        ...
        train(model, steps, rng, f"{name}-s{seed}")
        ALL[f"{name}_s{seed}"] = evaluate(model, has_cop=name != "transformer")
        del model
        if DEVICE == "cuda":
...
summary = dict(device=DEVICE, mode="FULL" if FULL else "SMOKE", cfg=CFG,
...
for run, r in ALL.items():
    ...
    print(f"{run:<22}" + "".join(cells) + f"  gate={gate}")
print("=" * 78)
