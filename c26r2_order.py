...
def run_program(Ph, Eh, p0h, tape, cap, nd=None):
    ...
    for n in range(1, cap + 1):
        ...
        if torch.equal(out, tape):
            return tape, n, tr, visits, cons
        tape = out
        tr.append(int((tape == MARK).sum()))
    return tape, cap, tr, visits, cons


def fitness(Ph, Eh, p0h, cases, seed, w_cons, w_slot, w_order=0.0):
    ...
    for nd in cases:
        ...
        fin, n, tr, v, cons = run_program(Ph, Eh, p0h, tape, nd + 9, nd=nd)
    ...
    N = len(cases)
    w_cons = w_cons - w_order
    return (0.10 * f_halt / N + 0.15 * f_disc / N + w_cons * f_cons / N
            + w_order * f_order / N + w_slot * f_slot / N, visits)
...
cases = cases_draw()
best, visits = fitness(Ph, Eh, p0h, cases, 0, 0.75, 0.0, 0.35)
print(f"[c26r2] seed fitness(stage A) = {best:.4f}", flush=True)
...
while edits < budgetA and no_improve < (400 if SMOKE else 15000):
    ...
    if edits % 25 == 0:
        cases = cases_draw()
    sc, v2 = fitness(Ph, Eh, p0h, cases, R.randrange(100000), 0.75, 0.0, 0.35)
    edits += 1
    if sc >= best - 1e-9:
        if sc > best + 1e-9:
            if sc - best >= 0.005:
                print(f"[c26r2] A edit {edits}: {best:.4f} -> {sc:.4f}", flush=True)
            no_improve = 0
        best = max(best, sc)
        visits = v2
        accepted += 1
        ...
        if kind == "cell":
            ...
            Eh[DIGS, h] = eold
        no_improve += 1
print(f"[c26r2] Stage A done: edits={edits} best={best:.4f}", flush=True)

# ---------------- Stage B: + slot credit ----------------
bestB, visitsB = fitness(Ph, Eh, p0h, cases, 0, 0.15, 0.60, 0.15)
...
while editsB < budgetB and no_improve < (400 if SMOKE else 20000):
    ...
    if editsB % 25 == 0:
        cases = cases_draw()
    sc, v2 = fitness(Ph, Eh, p0h, cases, R.randrange(100000), 0.15, 0.60, 0.15)
    editsB += 1
    if sc >= bestB - 1e-9:
        if sc > bestB + 1e-9:
            if scB_delta := sc - bestB >= 0.005:
                print(f"[c26r2] B edit {editsB}: {bestB:.4f} -> {sc:.4f}",
                      flush=True)
            no_improve = 0
        bestB = max(bestB, sc)
        visitsB = v2
        ...
        if kind == "cell":
            ...
            Eh[DIGS, h] = eold
        no_improve += 1
print(f"[c26r2] Stage B done: edits={editsB} best={bestB:.4f}", flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c26r2_searched.pt")
...
for ndmax in stages:
    ...
    for s in range(1000 if SMOKE else 4000):
        ...
        if s % 200 == 0:
            ok, n = probe(ndmax)
            print(f"[c26r2] C stage nd<={ndmax} s{s} lr={lr:.5f} "
            ...
            if ok >= 60:
                ...
                break
    if not stag:
        print(f"[c26r2] C stage nd<={ndmax} did not crystallize", flush=True)
if best_sd is not None:
    m.load_state_dict(best_sd)
    print(f"[c26r2] restored best-probe checkpoint ({best_ok}/64)", flush=True)
torch.save(m.state_dict(), "c26r2_order.pt")
...
out = dict(tag="ARC2-C26R2-P6-ORDER",
