...
ck = torch.load("c24d_searched.pt")  # CLEAN counter seed, not r3
Ph = torch.zeros(A, H, dtype=torch.long)
Eh = torch.zeros(A, H, dtype=torch.long)
Ph[:, :16] = ck["Ph"]
Eh[:, :16] = ck["Eh"]
for h in range(16, H):                     # new states: identity rows
    Ph[:, h] = torch.arange(H)[torch.arange(A) % H]
    Ph[:, h] = h                            # self-loop
    Eh[:, h] = torch.arange(A)
p0h = int(ck["p0h"])
...
for nd in cases0:
    ...
    _, _, _, _, _, cs = run_program(Ph, Eh, p0h, torch.tensor(make_tape(nd, digs)),
                                    nd + 9, nd=nd)
...
print(f"[c29b] bank cloned from entry state h{s_star}: {entry_count}", flush=True)
...
print(f"[c29b] seed fitness = {best:.4f}", flush=True)

budget = 400 if SMOKE else 80000
...
while edits < budget and no_improve < no_improve_cap:
    ...
    if mv < 0.30:                                   # transport MACRO move
        ...
        for (which, a, hh, old) in cells:
            if which == 0:
                Eh[a, hh] = BLK if a == d else DIG0 + (d - DIG0)
            else:
    ...
    if sc >= best - 1e-9:
        if sc > best + 1e-9:
            if sc - best >= 0.005 or sc >= 0.98:
                print(f"[c29b] edit {edits} (macros {macros}): {best:.4f} -> "
                      f"{sc:.4f}", flush=True)
            no_improve = 0
        best = max(best, sc)
        visits = v2
        ...
        if kind == "macro":
            ...
            Eh[DIGS, h] = eold
        no_improve += 1
print(f"[c29b] done: edits={edits} accepted={accepted} macros={macros} "
      f"best={best:.4f}", flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c29b_bank.pt")
...
out = dict(tag="ARC2-C29B-P6-BANK", search=dict(edits=edits, accepted=accepted,
