...
VAL = [gen_abcp(24, 64, torch.Generator().manual_seed(7600 + i)) for i in range(3)]
def score(tab, Ph):
    c = t = 0
    for x, y, _, _ in VAL:
...
for o in sorted(set(tab["o"])):
    print(f"[auto] op#{o} g={int(L0.gbits[o])} perm={parr(Ph, o)}", flush=True)
...
for L in (256, 1024, 4096):
    res[str(L)] = acc(gen_abcp, L, 4, 9500)
out = dict(tag="EXP068-ABCP-GENERIC-COMPILE", ckpt=CKPT, repairs=log, acc=res,
           certified=all(v == 1.0 for v in res.values()),
           wall_s=round(time.time() - t0, 1),
           peak_rss_mb=round(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/1024,1))
print("RESULT " + json.dumps(out), flush=True)
open("results.jsonl", "a").write(json.dumps(out) + "\n")
torch.save(dict(tab=tab, Ph=Ph, model=model.state_dict()), "oppram_abcp_compiled.pt")
