...
print(f"[c29f] bank cloned from entry state h{s_star}: {entry_count}", flush=True)
...
print(f"[c29f] seed fitness = {best:.4f}", flush=True)
    ...
    if sc >= best - 1e-9:
        if sc > best + 1e-9:
            if sc - best >= 0.005 or sc >= 0.98:
                print(f"[c29f] edit {edits} (macros {macros}): {best:.4f} -> "
                      f"{sc:.4f}", flush=True)
            no_improve = 0
        best = max(best, sc)
        visits = v2
        ...
        if kind == "macro":
            ...
            Eh[fam, h] = eold
        no_improve += 1
print(f"[c29f] done: edits={edits} accepted={accepted} macros={macros} "
      f"best={best:.4f}", flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c29f_bdig.pt")
...
out = dict(tag="ARC2-C29F-P6-BDIG", search=dict(edits=edits, accepted=accepted,
