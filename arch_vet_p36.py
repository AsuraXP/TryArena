...
import arch_vet_p13d as p13d, arch_vet_lm as lm, arch_vet_p32 as p32, arch_vet_p35 as p35
V0, VB = p26.V0, p26.VB
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--seeds", default="111,222,333"); ap.add_argument("--steps", type=int, default=1000); a = ap.parse_args()
    t0 = time.time(); out = {"tag": "ARCH-VET-LM-P36", "protocol": __doc__[:1900], "per_seed": {}}
    ...
    for seed in map(int, a.seeds.split(",")):
        ...
        mK = p35.KRBHind(V0, 24, 8, seen=True); mK.load_state_dict(torch.load(f"{CKPT}/P35_R2_SEEN_s{seed}.pt")["sd"])
        ...
        torch.save({"sd": gk.state_dict()}, f"{CKPT}/GK_s{seed}.pt"); gk.eval(); u.eval()
        with torch.no_grad():
        ...
        out["per_seed"][seed] = row; print(f"[p36 s{seed}] bars={row['n_bars']}/6 {row['guard']} identity={row['symbolic_dispatch_identity']} Kshare={row['K_share_on_krb']} text {row['routed_text_ce']}/{row['C_alone_text_ce']} params {row['params']}", flush=True)
