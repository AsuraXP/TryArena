...
def gen_batch(batch, kmax, dmax):
    ...
    for _ in range(batch):
        ...
        tapes.append(make_tape(k, gen_digits(max(k, 1), g)))
        ks.append(k)
...
def terminal_loss(x, ks):
    ...
    for b in range(B):
        ...
        for i in range(nd):
            y[b, nd + 1 + 2 * i] = BLK
            y[b, nd + 2 + 2 * i] = row[nd + 1 + 2 * i] + (BDIG0 - DIG0)
    msk = (y != PAD)
...
def probe(kmax, dmax, n=64):
    ...
    for _ in range(n):
        ...
        digs = gen_digits(max(nd, 1), gp)
        nd = len(digs)
        tape, np_, _ = m.run_fixpoint(torch.tensor(make_tape(nd, digs)), nd + 9)
        row = tape.tolist()
        ok += all(row[nd + 1 + 2 * i] == BLK and
                  row[nd + 2 + 2 * i] == BDIG0 + digs[i] for i in range(nd))
    return ok, n
...
for (kmax, dmax) in stages:
    ...
    for s in range(600 if SMOKE else 4000):
        ...
        if s % 200 == 0 or SMOKE and s % 100 == 0:
            ...
            print(f"[c29g] stage k<={kmax} s{s} lr={lr:.5f} CE {loss.item():.4f} "
            ...
            if ok >= 60:
                ...
                break
    if not stag:
        print(f"[c29g] stage k<={kmax} did not crystallize", flush=True)
if best_sd is not None:
    m.load_state_dict(best_sd)
    print(f"[c29g] restored best-probe checkpoint (probe {best_ok}/64)", flush=True)
torch.save(m.state_dict(), "c29g_sgd.pt")
...
print(f"[c29g] rows changed vs counter seed: {len(changed)}", flush=True)
counter_rows = [(a, h) for a in (MARK, BLK, SEP) for h in range(H)]
counter_changed = [c for c in changed if (c[0], c[1]) in counter_rows]
print(f"[c29g] counter-region rows changed: {len(counter_changed)} "
...
out = dict(tag="ARC2-C29G-P6-SGD", cert=res, verdict=v, steps=step,
