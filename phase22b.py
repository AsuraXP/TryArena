...
torch.save(model.state_dict(), f"isa_abcp_s{SEED}.pt")
for l in model.layers: l.hard = True
model.eval(); res = {}
with torch.no_grad():
    l = model.layers[0]
    h = model.emb(torch.arange(4))
    a = F.softmax(l.alpha(h), -1); rl = l.isa_onehot
...
out = dict(tag=f"EXP076-ISA-ABCP-S{SEED}", params=count_params(model), acc=res,
