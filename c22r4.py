"""C22-R round 3: chatbot repair — machine v9c (c22r4).
...
Round-3 residual diagnosis: host0 SSM decay log_a drifted to a=0.986
(r2/v8 max 0.83) and head norms blew up (11.7/18.3/21.6 vs ~10): with
wd=0 organs solving answers early, the host overfits 63-token-window
residuals using a slow-decay channel; at L>=256 the saturated SSM state
+ big head emit fixed wrong tokens (CE 17-22 at fill-A positions).

Fix here (F5): clamp SSM decay a <= A_MAX=0.90 in the hosts (mechanism
prior: organs own persistent state, hosts only need local context; the
clamp never binds the healthy r2/v8 regime, max a there = 0.826).

Wall budget: ~25 min. Tag ARC2-C22R4-REPAIR.
"""
...
def _clamped_ssm_forward(self, h):
    x = self.Wx(self.n1(h))
    a = torch.clamp(1.0 - torch.exp(-F.softplus(self.log_a)), max=A_MAX)
    B, L, d = x.shape
...
SSMBlock.forward = _clamped_ssm_forward
print(f"[c22r4] preamble loaded, SSM decay clamped <= {A_MAX} ({time.time()-T0:.0f}s)", flush=True)
...
print("[smoke-v9c] forward host0 = host + state organ + math organ OK", flush=True)
print("[smoke-v9c] PASSED", flush=True)
...
for step in range(1, STEPS + 1):
    ...
    if step % 2000 == 0:
        print(f"  [v9c] step {step}/{STEPS} lm {lm:.4f} rt {rt:.4f} "
              f"gates {[round(float(v), 2) for v in torch.exp(m.head_gate)]} "
              f"st_m_abs {float(m.st_m.abs().sum()):.1f} "
              f"({time.time()-t0:.0f}s)", flush=True)
torch.save(m.state_dict(), "c22r4.pt")
print(f"[v9c] trained in {time.time()-t0:.0f}s", flush=True)
...
print(f"[eval-v9c] {res}", flush=True)
dlg = dialogue_gen(m)
...
final = {"tag": "ARC2-C22R4-REPAIR",
         "changes": ["organ wd=0", "math turns in state family (MIX +18/118)",
                     "host0 branch gets math organ too", "16k steps",
                     "SSM decay clamp a<=0.90 (fixes r3 length-collapse: "
                     "log_a drift to 0.986 + head norm blowup)"],
         "v9c": res, "bars": bars, "D7_detail": D7,
