...
cases = [1, 2, 2, 3, 3, 3, 4, 4, 4, 4] if not SMOKE else [1, 2, 2]

# identity init
Ph = torch.arange(H).unsqueeze(0).expand(A, H).contiguous().clone()
Eh = torch.arange(A).unsqueeze(1).expand(A, H).contiguous().clone()
p0h = 0
best, visits = fitness(Ph, Eh, p0h, cases)
print(f"[c26a] identity-init fitness = {best:.4f}", flush=True)
...
while edits < budget and no_improve < no_improve_cap:
    ...
    if sc >= best - 1e-9:                             # neutral drift allowed
        if sc > best + 1e-9:
            if sc - best >= 0.005 or sc >= 0.98:
                print(f"[c26a] edit {edits}: {best:.4f} -> {sc:.4f}", flush=True)
            no_improve = 0
        best = max(best, sc)
        visits = v2
        accepted += 1
    ...
    if time.time() - last_rep > 120:
        print(f"[c26a] ... edits={edits} best={best:.4f} accepted={accepted}",
              flush=True)
        last_rep = time.time()
print(f"[c26a] done: edits={edits} accepted={accepted} best={best:.4f}",
      flush=True)
torch.save(dict(Ph=Ph, Eh=Eh, p0h=p0h), "c26a_searched.pt")
...
out = dict(tag="ARC2-C26A-P6-BIND", search=dict(edits=edits, accepted=accepted,
